...
export const ModuleBadReqDetector: React.FC = () => {
  ...
  const [sourceMode, setSourceMode] = useState<'preset' | 'project'>('preset');
  const [selectedDemoIndex, setSelectedDemoIndex] = useState<number>(0);
  const [isFixing, setIsFixing] = useState<boolean>(false);
  const [showResolved, setShowResolved] = useState<boolean>(false);

  const presetDemoCases = [
    {
      title: 'Vague Speed Benchmark',
      badInput: 'The system should provide reports quickly when requested by executives.',
      defects: [
        'DEF-01: Subjective term "quickly" cannot be verified in performance testing.',
        'DEF-14: Lacks quantifiable throughput limits or payload size bounds.',
        'DEF-17: Missing operational error handling timeout.'
      ],
      aiDetectionScore: '38% Quality Health',
      fixedIEEE: 'The administrative reporting engine shall generate and render analytical reports within 2.5 seconds for dataset queries up to 500,000 records under 1,000 concurrent sessions.'
    },
    {
      title: 'Ambiguous Usability & Security',
      badInput: 'The mobile app must be robust, easy to use, and protect passwords effectively.',
      defects: [
        'DEF-02: Buzzword "robust" without failure rate MTBF / MTTR bounds.',
        'DEF-02: Subjective term "easy to use" violates ISO/IEC 29148 testability.',
        'DEF-13: Unspecified cryptographic hashing standard for passwords.'
      ],
      aiDetectionScore: '25% Quality Health',
      fixedIEEE: 'The authentication service shall hash customer passwords using bcrypt (work factor >= 12), enforce TLS 1.3 encryption, and achieve a task completion rate >= 95% in <= 3 steps.'
    },
    {
      ...
      defects: [
        'DEF-12: Vague quantification "lots of users".',
        'DEF-01: Informal colloquial syntax "without server crashing".',
        'DEF-14: No SLA on request drop rates or timeout thresholds.'
      ],
      aiDetectionScore: '30% Quality Health',
      fixedIEEE: 'The checkout service shall sustain 100,000 concurrent active checkout sessions with an average response time < 800ms, 99.99% uptime, and 0% transaction loss rate.'
    }
  ];

  // Dynamic project cases
  const projectCases = (currentProject?.requirements || []).map(req => {
    const defects = req.issues.length > 0 
      ? req.issues.map(i => `${i.code ? i.code + ': ' : ''}${i.problem}`)
      : ['No severe defects flagged. Conforms to IEEE standards.'];
    const score = Math.max(20, 100 - req.issues.length * 25);

    return {
      title: `${req.id}: ${req.title.substring(0, 32)}...`,
      badInput: req.description,
      defects,
      aiDetectionScore: `${score}% Quality Health`,
      fixedIEEE: req.improvedText || AIEngine.generateContextualIEEERewrite(req.description, currentProject?.domain || 'General')
    };
  });

  const activeCases = sourceMode === 'project' && projectCases.length > 0 ? projectCases : presetDemoCases;
  const activeDemo = activeCases[selectedDemoIndex] || activeCases[0] || presetDemoCases[0];
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex items-center justify-between bg-surface/80 p-3 rounded-xl border border-white/10">
        <div className="flex items-center space-x-2">
          <button
            onClick={() => { setSourceMode('preset'); setSelectedDemoIndex(0); setShowResolved(false); }}
            className={`px-3 py-1.5 rounded-lg text-xs font-mono font-bold transition cursor-pointer flex items-center gap-1.5 ${
              sourceMode === 'preset' ? 'bg-rose-500/20 text-rose-300 border border-rose-500/40 shadow-neon-rose' : 'text-slate-400 hover:text-white'
            }`}
          >
            <ShieldAlert className="h-3.5 w-3.5" />
            <span>Preset Signature Demos (3)</span>
          </button>
          <button
            onClick={() => { setSourceMode('project'); setSelectedDemoIndex(0); setShowResolved(false); }}
            disabled={!currentProject || currentProject.requirements.length === 0}
            className={`px-3 py-1.5 rounded-lg text-xs font-mono font-bold transition cursor-pointer flex items-center gap-1.5 disabled:opacity-40 ${
              sourceMode === 'project' ? 'bg-cyan-500/20 text-cyan-300 border border-cyan-500/40 shadow-neon-cyan' : 'text-slate-400 hover:text-white'
            }`}
          >
            <FolderOpen className="h-3.5 w-3.5" />
            <span>Current Project Reqs ({currentProject?.requirements.length || 0})</span>
          </button>
        </div>

        <span className="text-[11px] font-mono text-slate-400">
          Target Context: <strong className="text-cyan-400">{currentProject?.domain || 'General'}</strong>
        </span>
      </div>

      {/* Demo Selector Tabs */}
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {activeCases.map((demo, idx) => (
          <button
            key={idx}
            onClick={() => { setSelectedDemoIndex(idx); setShowResolved(false); }}
            className={`p-4 rounded-xl border font-mono text-xs text-left transition-all duration-200 cursor-pointer ${
          ...
          >
            <span className="text-[10px] text-slate-400 block mb-1">
              {sourceMode === 'project' ? `PROJECT REQ ${idx + 1}` : `DEMO CASE 0${idx + 1}`}
            </span>
